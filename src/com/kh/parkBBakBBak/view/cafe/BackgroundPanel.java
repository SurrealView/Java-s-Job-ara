...
public class BackgroundPanel extends JPanel {
	...
	public BackgroundPanel(JFrame mf, Player p) {
		...
		JLabel cafeManager = new JLabel(
				new ImageIcon(new ImageIcon("images/cafeManager2.PNG").getImage().getScaledInstance(319, 243, 0)));
		...
		JButton goBack = new JButton(
				new ImageIcon(new ImageIcon("images/backButton.PNG").getImage().getScaledInstance(60, 60, 0)));
		...
		ta2.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				System.out.println("쿠폰카운트" + u.getCouponCount());
				if (couponGaesoo > 0) {
					if (num2 == 0) {
						...
						num2 = 0;
						u.setCouponCount(u.getCouponCount() + 1);

						if (u.getCouponCount() == 1) {
							HiddenPanel hidden = new HiddenPanel(mf, u);
							ChangePanel.replacePanel(mf, panel, hidden);

						} else if (u.getCouponCount() == 5) {
		...
		cafeManager.addMouseListener(new MouseListener() {
			...
			public void mouseExited(MouseEvent e) {
				...
				cafeManager.setIcon(new ImageIcon(
						new ImageIcon("images/cafeManager2.PNG").getImage().getScaledInstance(319, 243, 0)));
			}
			...
			public void mouseEntered(MouseEvent e) {
				...
				if (u.getPppppakk() == 0) {
					...
					cafeManager.setIcon(new ImageIcon(
							new ImageIcon("images/PPackPPack.PNG").getImage().getScaledInstance(360, 384, 0)));
				}
			}
		...
		for (int i = 0; i < remainBattery.length; i++) {
			batteryImage.add(new ImageIcon(ADDRESS + remainBattery[i]).getImage().getScaledInstance(140, 45, 0));
		}
		...
		/*
		 * JLabel batteryCase = addJLabelImage(new JLabel(), "batteryCase.png", 111,
		 * 44); batteryCase.setLocation(10, 5); String[] remainBattery = new String[] {
		 * "battery1.png", "battery2.png", "battery3.png", "battery4.png",
		 * "Battery5.png" };
		 * 
		 * ArrayList<Image> batteryImage = new ArrayList<Image>(); for (int i = 0; i <
		 * remainBattery.length; i++) { batteryImage.add(new ImageIcon(ADDRESS +
		 * remainBattery[i]).getImage().getScaledInstance(111, 44, 0)); }
		 * 
		 * ArrayList<JLabel> batteryLabels = new ArrayList<JLabel>();
		 * 
		 * if(u.getHp()!=0) { for (int i = 0; i < p.getHp(); i++) {
		 * batteryLabels.add(new JLabel(new ImageIcon(batteryImage.get(i))));
		 * batteryLabels.get(i).setSize(111, 44); batteryLabels.get(i).setLocation(10,
		 * 5); }
		 * 
		 * for(int i=0; i<u.getHp(); i++) { this.add(batteryLabels.get(i)); } }
		 * this.add(batteryCase);
		 */
